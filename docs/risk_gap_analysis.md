...
| Text-only PDF may be mistaken for a formal inspection report. | Low | User / Government / Legal | The current advice and risk basis can be downloaded without photos or debug metadata; every PDF states the one-photo/visible-range limit, possible omissions, professional-judgment boundary, and non-persistence. | Keep the disclaimer prominent, never add approval/signature language, and require on-site or professional confirmation where appropriate. | No for first inquiry; re-review before any pilot. |
