...
The prompt requires direct, minimal visual evidence. A feature may be `absent_with_full_coverage` only when the relevant area is fully visible. Cropped, obscured, or ambiguous evidence must be `cannot_determine` and produces no output item. Negative observation of a bathroom chair uses the expected key `has_shower_chair`, not a negated ontology key.
...
After parsing, `RelationshipEngine` verifies room-scoped ontology membership and the configured subject/predicate/target triple. For localized visible evidence, it resolves the exact `(room, ontology_key, visible_hazard)` rule and emits a visible finding. `RuleEngine` then uses that rule to assign severity, Japanese wording, source IDs, model-score threshold treatment, and actions from `room_checklists.yaml`; duplicate `risk_type` values cannot redirect this path. The compatible `confidence` field is an uncalibrated model signal, not a probability of correctness. Gemini cannot override the deterministic policy or its three action tiers.
...
With `room_hint=auto`, the first provider call uses the compact generic facts prompt. If it identifies a known home room but yields no localized visible hazard, one bounded follow-up reviews only that room's checklist. The follow-up must keep the same room and stay within that room's entity vocabulary. Valid localized visible hazards survive even when the expected-feature channel is incomplete; individually invalid expected-feature observations are discarded and cannot suppress the follow-up.
