...
| Identity | Canonical findings, `result_key`, and `semantic_hash` separate calculation inputs from reader-facing semantics. `result_key` includes `schema_version`; same-class evidence boxes with IoU at least 0.5 are deterministically deduplicated. | A schema change changes computation identity; duplicate detections do not duplicate semantics/actions. Metadata, display bbox, images, timing, and mode do not alter semantic identity. |
| Rendering | Positive in-frame evidence bbox coordinates drive danger selection, overlap suppression, and annotated red boxes. Presentation mapping is improvement-image-only. | Visual zones cannot move or replace provider evidence. |
...
When the browser cannot reach the backend in non-strict local mode, `local_mock` returns a neutral not-applicable abstention with empty findings/actions and identical unannotated images. It does not fabricate a medium-risk box or recommendations. This is separate from the agent's intentional deterministic mock mode used by tests and credential-free POC runs.
